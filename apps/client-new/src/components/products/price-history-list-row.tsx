...
const PriceHistoryListRow = ({
  ...
  const handleSetActivePrice = async (id: string) => {
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        {dayjs(priceHistory.createdAt).format("DD/MM/YYYY")}
      </TableCell>
      <TableCell>{priceHistory.purchasedPrice}</TableCell>
      <TableCell>{priceHistory.price}</TableCell>
      <TableCell>
        {priceHistory.id === activePriceId ? (
          <CustomChip label="Active" status="info" />
        ) : (
          <Button
            disabled={updateLoading}
            onClick={() => handleSetActivePrice(priceHistory.id)}
          >
            {updateLoading && (
              <CircularProgress
                size={16}
