...
import { NOTIFICATIONS_BY_USERID, NotificationByUserIdData, NotificationByUserIdVars } from "@/graphql/notifications/queries";
...
export const NotificationsButton = () => {
    ...
    const{data,error,loading} = useQuery<NotificationByUserIdData,NotificationByUserIdVars>(NOTIFICATIONS_BY_USERID,{
        variables:{
    ...
    const unread = data?.allNotificationsByUserId.filter((notification)=>{
        if(notification.recipientType === "USER"){
            ...
            return !notification.notificationReads.some((n)=>n.userId === (sessionData?.user as any).id || "")
        }
    })
